...
8. **SQL Editor** → New Query → paste contents of `supabase/migrations/0006_manual_activation.sql` → Run (adds manual push, expanded channels, auto-fire toggle)
9. **Database → Replication** → enable replication for `cdp_user_profiles`, `cdp_activations`, `cdp_events`
10. **Settings → API Keys** → copy: Project URL, **publishable key** (`sb_publishable_...`), **secret key** (`sb_secret_...`)
